...
class generate_network_ready_files():
    def __init__(self,vec_path,processed_data_path,word_vec_size,max_q_length,max_option_length,max_opt_count,max_sent_para,max_words_sent,op_path=None):
    ...
    def get_vec_for_word(self,model, word):
        try:
            # BERT-embdeddings
            # vec = extract_embeddings(model, unicode(word))
            # vec_out = np.sum(vec[0], axis=0)[:300]
            vec = model[word]
            # print(vec_out.shape)
            return vec
        except:
    ...
    def write_vecs_to_file(self,model,raw_data_content,word2vec_file,is_closest_para_file = False):
        ...
        if is_closest_para_file:
            ...
            for sent in raw_data_content:
                ...
                words = [w for w in words if w not in string.punctuation]   ## to remove punctuations from closest_sent.txt
                for word in words:
    ...
    def generate_word2vec_for_all(self):
        ...
        # load word2vec model
        model = KeyedVectors.load_word2vec_format(self.vec_path, binary=True, limit=500000)

        # load fasttext model
        # model = FastText.load_model(self.vec_path)
        ...
        for folder in self.folder_list:
            ...
            for question_dir in questions_dir:
                ...
                for fname in file_list:
                    ...
                    self.write_vecs_to_file(model,raw_data_content,f,is_closest_para_file)
                    f.close()
            print 20*"***"
