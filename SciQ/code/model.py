...
# get_cnn_model1 refers to cnn_{3,4,5}
# get_cnn_model2 refers to cnn_{2,3,4} ORIGINAL MODEL 
# get_cnn_model3 refers to cnn_{2,3} SCALED DOWN MODEL
# get_cnn_model4 refers to cnn_{2,3,4,5} AUGMENTED MODEL
...
class sciq_model():
    ...
    def get_cnn_model3(self):
        ...
        final_weights = Lambda(lambda xin: xin/K.cast(K.sum(xin, axis=1, keepdims=True), K.floatx()),output_shape=(self.max_sent_para,self.max_opt_count))
        mask_weights = Lambda(lambda xin: T.switch(T.eq(xin,0),np.NINF,xin), output_shape=(self.max_sent_para,self.max_opt_count))
        glob_pool = Lambda(lambda xin: K.mean(xin, axis=[1, 2]),output_shape=(150,)) #100
        ...
        meanpool_0 = glob_pool(conv_0)
        meanpool_1 = glob_pool(conv_1)
        concatenated_tensor = Concatenate(axis=1)([meanpool_0, meanpool_1])
    ...
    def get_cnn_model4(self):
        ...
        conv_1 = Conv2D(num_filters, kernel_size=(filter_sizes[1], self.word_vec_size), padding='valid', kernel_initializer='normal', activation='linear')(cnn_reshape)
        conv_2 = Conv2D(num_filters, kernel_size=(filter_sizes[2], self.word_vec_size), padding='valid', kernel_initializer='normal', activation='linear')(cnn_reshape)
        conv_3 = Conv2D(num_filters, kernel_size=(filter_sizes[3], self.word_vec_size), padding='valid', kernel_initializer='normal', activation='linear')(cnn_reshape)

        meanpool_0 = glob_pool(conv_0)
        meanpool_1 = glob_pool(conv_1)
        meanpool_2 = glob_pool(conv_2)
        meanpool_3 = glob_pool(conv_3)
        concatenated_tensor = Concatenate(axis=1)([meanpool_0, meanpool_1, meanpool_2, meanpool_3])
